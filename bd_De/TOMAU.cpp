...
bool check(ii A , ii B , ii C) {
    return double(A.fi) * (B.se - C.se) + double(B.fi) * (C.se - A.se) + double(C.fi) * (A.se - B.se) > 0;
}
...
signed main() {
    ...
    int cnt , dem = 0 , OK , l , r , mi;
    ii res;
    cin >> q;
    FOR(i , 1 , q) {
        cin >> res.fi >> res.se;
        res.fi ^= dem * dem * dem * t;
        res.se ^= dem * dem * dem * t;
        cnt = 1; OK = 0;
        while (cnt + (n >> 1) + 1 <= (n << 1) ) {
            if(check(a[cnt] , a[cnt + 1] , res) == 1){
                if(check(a[cnt + (n >> 1)] , a[cnt + (n >> 1) + 1] , res) == 1){
                    OK = 1;
                    dem += 1;
                    cout << "YES" << endl;
                    break;
                ...
                if(check(a[cnt + (n >> 1)] , a[cnt + (n >> 1) + 1] , res) == 0) cnt += (n >> 1) + 1;
                else {
                    l = cnt + 1 , r = cnt + (n >> 1);
                    while(l <= r){
                        mi = ((l + r) >> 1);
                        if(check(a[mi] , a[mi + 1] , res)) {
                            cnt = mi;
                            r = mi - 1;
                        }
                        else l = mi + 1;
