...
signed main() {
    ...
    cin >> n;
    FOR(i , 1 , n * (n - 1) / 2)cha[i] = i;
    FOR(i , 1 , n)cin >> a[i].fi >> a[i].se;
    FOR(i , 1 , n - 1){
        FOR(j , i + 1 , n){
            canh.pb({i , j , kc(a[i] , a[j])});
        }
    }
    sort(canh.begin() , canh.end() , cmp);
    int ans = 0;
    REP(i , canh.size()){
        if (ghep(canh[i].u, canh[i].v) == true)ans = max(ans , canh[i].val);
    }
    n = (int)(sqrt(ans) / 2 * 10000000);
    int dem = 0;
    string sum = "";
    while(n > 0){
        dem += 1;
        sum = (char)(char(n % 10) + 48) + sum;
        n /= 10;
        if(dem == 7)break;
    }
    cout << n << '.' << sum;
