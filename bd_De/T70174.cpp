...
signed main() {
    freopen("T70174.inp", "r", stdin);
    freopen("T70174.out", "w", stdout);
    ...
    FORD(i , n , 1){
        ans += abs(F[i].fi - F[i].se);
        if(F[cha[i]].fi == 0) F[cha[i]].fi = 2 * max(F[i].fi , F[i].se);
        else F[cha[i]].se = 2 * max(F[i].fi , F[i].se);
    }
