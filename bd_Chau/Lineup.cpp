...
signed main() {
    ...
    int dem = 1;
    FOR(i , 2 , n){
        if(a[i].se != a[i - 1].se)dem += 1;
    }
    b[1] = 1;
    FOR(i , 2 , n){
        if(a[i].se > a[i - 1].se)b[i] = b[i - 1] + 1;
        else b[i] = b[i - 1];
    }
    FOR(i , 1 , n)a[i].se = b[i];
    sort(a + 1 , a + 1 + n);
    int l = 1 , r = 1 , res = 0 , ans = oo;
    while(r <= n){
        dd[a[r].se] += 1;
        if(dd[a[r].se] == 1)res += 1;
        while(dd[a[l].se] > 1) {
            dd[a[l].se] -= 1;
            l += 1;
        }
        if(res == dem)ans = min(a[r].fi - a[l].fi , ans);
        r += 1;
    }
    cout << ans;
    return 0;
