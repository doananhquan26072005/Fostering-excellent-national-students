...
signed main() {
    ...
    sort(a + 1 , a + 1 + n , cmp);
    int i = 1 , y;
    S.push({a[1].fi , a[1].se});
    while(i < n){
        if(a[i + 1].fi > S.top().se)S.push({a[i + 1].fi , a[i + 1].se});
        else if(a[i + 1].se <= S.top().se){
            S.pop();
            S.push({a[i + 1].fi , a[i + 1].se});
        }
        else if(a[i + 1].fi <= S.top().se){
            y = S.top().se;
            S.pop();
            S.push({a[i + 1].fi , y});
        }
    ...
    cout << S.size();
    // while(!S.empty())cout << S.top().fi << " " << S.top().se << endl , S.pop();
    return 0;
}
/*
╔══╗                ▐▀▄       ▄▀▌   ▄▄▄▄▄▄▄
