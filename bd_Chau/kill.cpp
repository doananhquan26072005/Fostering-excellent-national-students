...
multiset <int> S;
struct dausi{
    int pow , coin , ans , stt;
} a[100005];

bool cmp(dausi x , dausi y){
    return x.pow < y.pow;
}

bool cmpp(dausi x , dausi y){
    return x.stt < y.stt;
}
...
signed main() {
    ...
    cin >> n >> k;
    FOR(i , 1 , n)cin >> a[i].pow , a[i].stt = i;
    FOR(i , 1 , n)cin >> a[i].coin;
    sort(a + 1 , a + 1 + n , cmp);
    int sum = 0;
    FOR(i , 1 , n){
        S.insert(a[i].coin);
        sum += a[i].coin;
        if(S.size() > k + 1)sum -= *S.begin() , S.erase(S.begin());
        a[i].ans = sum;
