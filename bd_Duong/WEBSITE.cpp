...
const int MOD = 1e9 + 7;
//----------------------------------------------------------------------
int t , n , dem, ans , check , cnt , soluong;
string s , s1;
map <string , int> dd;
int con[100005];
int p[100005];
...
signed main() {
    ...
    while(t --){
        ...
        FOR(i , 1 , n){
            cin >> s >> s1;
            if(s == s1)continue;
            if(dd[s] == 0)dd[s] = dem , dem += 1;
            if(dd[s1] == 0)dd[s1] = dem , dem += 1;
            con[dd[s]] = dd[s1];
        }
        FOR(i , 1 , dem)p[i] = 0;
        FOR(i , 1 , dem - 1){
            if(p[i] == 1)continue;
            check = 0; cnt = i; soluong = 0;
            while(1){
                if(p[cnt] == 1){
                    check = 0;
                    break;
