...
bool cmp(diem k, diem l) {
    return k.x * k.x + k.y * k.y < l.x * l.x + l.y * l.y;
}
...
signed main() {
    ...
    while (t --) {
        ...
        int ans = 0 , check = 0;
        FOR(i , 1 , n) {
            if (a[i].x * a[i].x + a[i].y * a[i].y == a[i + 1].x * a[i + 1].x + a[i + 1].y * a[i + 1].y)ans += a[i].val;
            else {
                ans += a[i].val;
                if (ans >= p) {
                    int k = sqrt(a[i].x * a[i].x + a[i].y * a[i].y);
                    if (k * k == a[i].x * a[i].x + a[i].y * a[i].y)cout << k << endl;
                    else cout << k + 1 << endl;
                    check = 1;
                    break;
                }
